...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function toggleMenu(){
      var menuSVG = document.querySelector('.ham');
      menuSVG.classList.toggle('hamActive')
      var menuList = document.querySelector('.menu_list');
      menuList.classList.toggle('menu_list_open')
      
  
      var menuItems = document.querySelectorAll('.menu_list_items');
      for(var i = 0; i < menuItems.length; i++) {
          var delayItemMove = toggleMenuItemsClass(i);
          setTimeout(delayItemMove, i*150); 
        } 
        function toggleMenuItemsClass( i ) {
          var item = menuItems[i];
          return function() {
            item.classList.toggle('menu_list_items_show');
          }
        }   
  };
